import { formatEther, formatUnits, parseAbi, encodeFunctionData } from 'viem'
...
// Вспомогательная функция для форматирования wei
function formatWei (wei: bigint): string {
  return (Number(wei)).toFixed(0)
}
...
export class WheelXSwap {
  ...
  private async approveTokens (amount: bigint): Promise<void> {
    try {
      logger.info('Одобряем использование токенов роутером...')
      ...
      logger.info(`Оценка газа для апрува: ${gasEstimate}`)
      logger.info(`Цена газа: ${formatUnits(gasPrice, 18)} ETH`)
      ...
      const tx = await this.walletClient.writeContract({
        ...
        functionName: 'approve',
        args: [UNISWAP_ROUTER_ADDRESS as `0x${string}`, amount],
        gas: gasEstimate * 120n / 100n, // +20% к оценке
        gasPrice: gasPrice,
        nonce: nonce,
        chain: soneiumChain,
      ...
      logger.info(`Транзакция апрува отправлена: ${tx}`)

      // Ждем подтверждения
      const receipt = await this.publicClient.waitForTransactionReceipt({
        hash: tx
      })
      logger.info(`Апрув подтвержден: ${receipt.status === 'success' ? 'success' : 'failed'}`)
  ...
  async executeTransaction (transactionRequest: {
    ...
    try {
      ...
      logger.info(`⛽ Оценка газа: ${estimatedGas}`)
      logger.info(`⛽ Лимит газа: ${gasLimit}`)
      ...
      logger.info(`✅ Транзакция отправлена как EIP-1559 (Type 2): ${hash}`)

      // Ждем подтверждения
      const receipt = await this.publicClient.waitForTransactionReceipt({ hash })

      if (receipt.status === 'success') {
        // Рассчитываем стоимость газа в ETH
        ...
        logger.info(`⛽ Использовано газа: ${receipt.gasUsed} (из ${gasLimit})`)
        logger.info(`⛽ Стоимость газа: ${gasCostETH} ETH`)
        logger.info(`⛽ Эффективная цена: ${formatWei(BigInt(receipt.effectiveGasPrice || '0'))} wei (${formatGwei(BigInt(receipt.effectiveGasPrice || '0'))} gwei)`)
