...
const SPENDERS = {
  AAVE_L2_POOL: '0xdd3d7a7d03d9fd9ef45f3e587287922ef65ca38b', // Используется и Aave, и Sake Finance
...
const SPENDER_NAMES: Record<string, string> = {
  [SPENDERS.AAVE_L2_POOL]: 'Aave/Sake Finance L2Pool',
...
export async function performRevoke (privateKey: `0x${string}`): Promise<{
  ...
  try {
    ...
    logger.info(`Найдено апрувов: ${approvals.length}`)
    ...
    logger.info(`✅ Отозвано: ${revokedCount}, ⏭️ Пропущено: ${skippedCount}, ❌ Ошибок: ${errorCount} из ${approvals.length}`)
