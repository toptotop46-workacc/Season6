...
export class TransactionChecker {
  ...
  async checkWallets (wallets: string[]): Promise<{
    ...
    logger.info(`Проверка ${wallets.length} кошельков параллельно...`)
    ...
    logger.info(`Активных кошельков: ${activeWallets.length}`)
    logger.info(`Завершенных кошельков: ${completedWallets.length}`)
